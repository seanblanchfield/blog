...
1. Viewing a real-time stream over TCP (i.e., HTTP) is not optimal. Any congestion between the camera, the router, the Home Assistant host and the web browser will lead to dropped traffic, which TCP will then spend seconds trying to recover (instead of forgetting about the old frames, and focussing on the current ones). The optimal solution is to display the original RTSP stream (which is UDP, not TCP) in the web browser. Unfortunately, web browsers do not support viewing raw RTSP streams. This is why Home Assistant is transcoding them in the first place.
